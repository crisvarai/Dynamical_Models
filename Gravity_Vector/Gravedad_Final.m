...
syms g
%-- Cordenada %Pi=(xo,yi,zi) del cm desde la referencia Oi, i=1,2,3,4,5,6 
syms x1 y1 z1   
syms x2 y2 z2
syms x3 y3 z3
syms x4 y4 z4
...
%--- SUBSTITUCIÓN DE VALORES, OBTENCIÓN DE G(q) [Nm] 
   ...
   cm1=[-106.41; -0.92; 49.68]/1000;
   cm2=[-199.44; 1.53; 151.77]/1000;
   cm3=[-294.44; 41.45; 222.04]/1000;
   cm4=[-399.42; -0.01; 291.26]/1000;
   cm5=[-458.11; -0.39; 318.81]/1000;
   cm6=[-600.99; -6.07; 371.74]/1000;

  %---- posición p_i del Cm respecto al sistema de referencia i
  P1=-transpose(R10)*O10 + transpose(R10)*cm1;
  P2=-transpose(R20)*O20 + transpose(R20)*cm2;
  P3=-transpose(R30)*O30 + transpose(R30)*cm3;
  P4=-transpose(R40)*O40 + transpose(R40)*cm4;
  P5=-transpose(R50)*O50 + transpose(R50)*cm5;
  P6=-transpose(R60)*O60 + transpose(R60)*cm6;

  %---- Posiciones P_i numéricas del cm respecto al sistema i [m]
  P1=double(subs(P1,[d1, a2, d3, a3, a4, d5, d7, k, q1, q2, q3, q4, q5, q6],[d_1, a_2, d_3, a_3, a_4, d_5, d_7, k_0, 0, 0, 0, 0, 0, 0]));
  P2=double(subs(P2,[d1, a2, d3, a3, a4, d5, d7, k, q1, q2, q3, q4, q5, q6],[d_1, a_2, d_3, a_3, a_4, d_5, d_7, k_0, 0, 0, 0, 0, 0, 0]));
  P3=double(subs(P3,[d1, a2, d3, a3, a4, d5, d7, k, q1, q2, q3, q4, q5, q6],[d_1, a_2, d_3, a_3, a_4, d_5, d_7, k_0, 0, 0, 0, 0, 0, 0]));
  P4=double(subs(P4,[d1, a2, d3, a3, a4, d5, d7, k, q1, q2, q3, q4, q5, q6],[d_1, a_2, d_3, a_3, a_4, d_5, d_7, k_0, 0, 0, 0, 0, 0, 0]));
  P5=double(subs(P5,[d1, a2, d3, a3, a4, d5, d7, k, q1, q2, q3, q4, q5, q6],[d_1, a_2, d_3, a_3, a_4, d_5, d_7, k_0, 0, 0, 0, 0, 0, 0]));
  P6=double(subs(P6,[d1, a2, d3, a3, a4, d5, d7, k, q1, q2, q3, q4, q5, q6],[d_1, a_2, d_3, a_3, a_4, d_5, d_7, k_0, 0, 0, 0, 0, 0, 0]));
  
  %---- Posiciones P_i del centro de masa respecto al sistema i [m]

  %     P11=[0.0546; 66.2941; -0.9200]/1000
  %     P22=[-5.0167; 1.5300; 61.1648]/1000
  %     P33=[ -0.2391; 67.4386; 41.4500]/1000
  %     P44=[-0.4010; -0.0100; 56.9593]/1000
  %     P55=[-0.2357; 1.2364; -0.3900]/1000
  %     P66=[-12.6485; -6.0700; 0.6260]/1000
  ...
  q_2=-115*pi/180;        % -45<q2<125
  q_3=-90*pi/180;        % -45<q3<180
  q_4=0*pi/180;          %  0<q4<135
  q_5=90*pi/180;          %  -180<q5<180
  q_6=0*pi/180; 


%---- Vector de gravedad para un punto en el espacio articular G(q1,q2,q3,q4,q5,q6)
